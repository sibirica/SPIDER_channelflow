...
cs = zeros( 14, 14, numel(k_max) );

for i = 1:numel(k_max)
  location = "center";
  tic
  cutoff = [ k_max(i), k_max(i), k_max(i) ];
  [center_G, labels, center_scales] = vector_library_function_w_noise( noise, cutoff, envelope_power, location, size_vec, seed_train );
  toc
  ...
  scales(:,i) =  center_scales;
end

save("results_noise_" + noise + ".mat");
end
...
x_vals = k_max/64; %l_corr / 32

scatter( x_vals, dimensionless(1,:), ms, 'filled', 'o', "MarkerFaceColor", colors{1});
scatter( x_vals, dimensionless(2,:), ms, 'filled', '>', "MarkerFaceColor", colors{2});
scatter( x_vals, dimensionless(3,:), ms,           'o', "MarkerFaceColor", colors{3}, "MarkerEdgeColor", "black", "linewidth", 2);
scatter( x_vals, dimensionless(4,:), ms,           '>', "MarkerFaceColor", colors{4}, "MarkerEdgeColor", "black", "linewidth", 2);
%xlim([0.8,12]);
%xticks([1, 2, 4, 8]);
set(gca, 'yscale', 'log')
set(gca, 'xscale', 'log')
xlabel("$l_c/l_{domain}$", "interpreter", "latex");
%ylabel("relative error", "interpreter", "latex");%, "rotation", 90);
%title("noise = 0.1");
...
saveas(gcf, "figures/c_noise0.1.pdf");
return
%}
