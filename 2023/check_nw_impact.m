...
location = "center";
tic
%cutoff = [ k_max(i), k_max(i), k_max(i) ];
noise = 0.0;
...
%%
steps = 16;
nw = exp( linspace(log(8), log(512-1), steps) );
nw = round(nw);
ns = 0*nw;
res= 0*nw;
cs = zeros(14,14,numel(nw));
...
for i = 1:numel(nw)
  cs_trials = zeros(14,14,trials);
  for j = 1:trials
    windows = randsample( size(center_G,1)/3-1, nw(i), false );
    windows = (windows-1)*3 + 1;
    size(windows)
    idx = [windows; windows + 1; windows + 2]; %get x y z components

    %override idx
    %idx = 1:3*nw(i);
    size(idx)

    %sample fewer windows
    G = center_G( idx, : );

    [center_NS_res_ave, res_std, center_NS_cs_ave, center_NS_cs_std] = compact_regression( G, labels );
    cs_trials(:,:,j) = center_NS_cs_ave;
  end
  ...
  %record output
  ns(i) = n;
  res(i) = center_NS_res_ave( 1, n );
  cs(:,:,i) = mean( abs(cs_trials), 3 );
end
save("results_nw.mat");
return;
...
dimensionless = abs(abs( cs./target) - 1);
...
% Create a cell array with these RGB values
colors = {darkGreen, darkBlue, darkRed, darkPurple};
colors = {"black", "black", "w", "w"};

%nw = nw/2;
scatter( nw, dimensionless(1,:), ms, 'filled', 'o', "MarkerFaceColor", colors{1});
scatter( nw, dimensionless(2,:), ms, 'filled', '>', "MarkerFaceColor", colors{2});
scatter( nw, dimensionless(3,:), ms,           'o', "MarkerFaceColor", colors{3}, "MarkerEdgeColor", "black", "linewidth", 2);
scatter( nw, dimensionless(4,:), ms,           '>', "MarkerFaceColor", colors{4}, "MarkerEdgeColor", "black", "linewidth", 2);
...
ylabel("$\varepsilon_n$", "interpreter", "latex", "rotation", 0);
%title("noise = 0.1");

m = numel(nw);
p = polyfit( log(nw(m/2:end)), log(dimensionless(4,m/2:end)), 1  )

%hold on
%  plot( nw, , "linewidth", 2, "color", "red" );
%hold off

xticks([16. 4*16, 16*16]);
xlim([16, 256]);
...
%exportgraphics(gcf, "figures_nw/coeffs_nw.pdf");
saveas(gcf, "figures/c_nw.pdf");
return
