...
function [G, labels, scales] = vector_library_function_w_noise( noise, k_max, envelope_power, location, size_vec, seed )
...
%{
corr = @(x,y) sum(x.*y, 'all') / sqrt( sum(x.^2,'all') * sum(y.^2, 'all') );
n = 20;
c = zeros(n,1);
dim = 1;
for i = 1:n
  i
  c(i) = corr( W, circshift(W,i,dim) );
end
