...
cs = zeros(14,14,numel(subsamples));
c_scales = zeros( 14, numel(subsamples) );
...
for i = 1:numel(subsamples)
  ...
  noise = 0;
  [center_G, labels, c_scales(:,i)] = vector_library_function_w_subsampling( noise, subsamples(i), envelope_power, location, size_vec, seed_train );
  toc
  ...
  print_table_entry( center_NS_cs_ave, center_NS_cs_std, center_G, n, c_scales(:,i), labels );

  %record output
  ns(i) = n;
  res(i) = center_NS_res_ave( 1, n );
  cs (:,:,i) = center_NS_cs_ave;
  css(:,:,i) = center_NS_cs_std;
...
kick = 0.;
scatter( subsamples+kick, dimensionless(1,:), ms, 'filled', 'o', "MarkerFaceColor", colors{1});
scatter( subsamples+kick, dimensionless(2,:), ms, 'filled', '>', "MarkerFaceColor", colors{2});
scatter( subsamples-kick, dimensionless(3,:), ms,           'o', "MarkerFaceColor", colors{3}, "MarkerEdgeColor", "black", "linewidth", 2);
scatter( subsamples, dimensionless(4,:), ms,           '>', "MarkerFaceColor", colors{4}, "MarkerEdgeColor", "black", "linewidth", 2);
...
xticks([1:10]);
xlim([1-2*kick,4+2*kick]);
set(gca, "color", "w");
leg = labels(idx);
for i = 1:numel(idx)
  leg{i} = "$" + leg{i} + "$";
end
leg{1} = ""; %kill the time derivative.
...
saveas( gcf, "figures/c_ss.pdf" );
%exportfig( gcf, 'figures_ss/c_ss.pdf' );
return
